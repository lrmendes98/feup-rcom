...
char* readDataPacket(char* packet, char* file_ptr) {
    int data_size = 0;
    packet += 2;
    data_size += (uint8_t)*packet * 256;
    packet++;
    data_size += (uint8_t)*packet;
    packet++;
...
int getFileSize(char* number_start) {
    int size = 0;
    size += (uint8_t)*number_start * 16777216;
    number_start++;
    size += (uint8_t)*number_start * 65536;
    number_start++;
    size += (uint8_t)*number_start * 256;
    number_start++;
    size += (uint8_t)*number_start;
    return size;
}
